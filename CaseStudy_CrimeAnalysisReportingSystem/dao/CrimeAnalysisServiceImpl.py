...
class CrimeAnalysisServiceImpl(ICrimeAnalysisService):
    ...
    def add_victim(self, victim: Victim) -> bool:
        ...
        try:
            ...
            """
            values = (
                victim.get_victim_id(),
                victim.get_first_name(),
                victim.get_last_name(),
                victim.get_date_of_birth().strftime('%Y-%m-%d'),
                victim.get_gender(),
                victim.get_contact_info()
            )
    ...
    def update_incident_status(self, status, incident_id: int) -> bool:
        ...
        try:
            ...
            query = "UPDATE Incidents SET Status = ? WHERE IncidentID = ?"
    ...
    def get_incidents_in_daterange(self, start_date: str, end_date: str):
        ...
        try:
            cursor = self.connection.cursor()
            query = "SELECT * FROM Incidents WHERE IncidentDate BETWEEN ? AND ?"
    ...
    def get_incidents_by_type(self, incident_type: str):
        ...
        try:
            cursor = self.connection.cursor()
            query = "SELECT * FROM Incidents WHERE IncidentType = ?"
    ...
    def get_case_details(self, case_id: int) -> Case:
        ...
        try:
            cursor = self.connection.cursor()
            query = "SELECT * FROM Cases WHERE CaseID = ?"
    ...
    def update_case_status(self, case_id: int, new_status: str) -> bool:
        ...
        try:
            cursor = self.connection.cursor()
            query = "UPDATE Cases SET CaseStatus = ? WHERE CaseID = ?"
    ...
    def get_officer_by_id(self, officer_id: int) -> Officer:
        ...
        try:
            cursor = self.connection.cursor()
            cursor.execute("SELECT * FROM Officers WHERE OfficerID = ?", (officer_id,))
            row = cursor.fetchone()
            if row:
                return Officer(*row)
            else:
    ...
    def get_recent_incidents_with_officer_details(self):
        ...
        try:
            cursor = self.connection.cursor()
            query = """
                SELECT i.IncidentID, i.IncidentType, i.IncidentDate, o.OfficerName
                FROM Incidents i
    ...
    def search_incidents(self, keyword: str):
        ...
        try:
            ...
            query = """
                ...
                WHERE Description LIKE ? OR IncidentType LIKE ?
            """
            param = f"%{keyword}%"
